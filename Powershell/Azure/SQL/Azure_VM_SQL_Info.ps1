...
$outputVmDisk = "c:\azure_vmdisk_info-$($date.ToString("yyyy-MM-dd_HHmm")).csv"
$outputSQL = "c:\azure_sql_info-$($date.ToString("yyyy-MM-dd_HHmm")).csv"
...
$vmList | Export-CSV -path $outputVmDisk -Append
Write-Host "SQL CSV file output to: $outputSQL" -foregroundcolor green
$sqlList | Export-CSV -path $outputSQL -Append
}
