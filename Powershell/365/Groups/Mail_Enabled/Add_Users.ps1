...
# Base Varibles
$csvFilePath = "C:\Users\Rhys's Desktop\Desktop\New Scripts\365 Group\Create.csv" # Define the CSV file path directly
...
foreach ($user in $usersList) {
    try {
        ...
        if (-not ((Get-UnifiedGroupLinks -Identity $groupId -LinkType Members).PrimarySmtpAddress -contains $member)) {
            ...
            Write-Warning "[WARNING] $member already exists in Mail-Enabled Security Group $groupId"
