...
$appsToRemove = $config.Config.RemoveApps.App
foreach ($app in $appsToRemove) {
    Write-Host "Removing app: $app"
    Get-AppxPackage -Name $app -AllUsers | ForEach-Object {
        try {
            $_ | Remove-AppxPackage -ErrorAction Stop
            Write-Host "App removed: $app"
        } catch {
            Write-Host "Error removing app: $app. $_"
        }
    }

    $provisionedApps = Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -eq $app }
    foreach ($provisionedApp in $provisionedApps) {
        try {
            Write-Host "Removing provisioned app: $app"
            Remove-AppxProvisionedPackage -Online -PackageName $provisionedApp.PackageName -ErrorAction Stop | Out-Null
            Write-Host "Provisioned app removed: $app"
        } catch {
            Write-Host "Error removing provisioned app: $app. $_"
        }
    }
}
